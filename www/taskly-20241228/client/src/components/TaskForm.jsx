...
export default function TaskForm({ type, task }) {
  ...
  const doSubmit = async (values) => {
    if (type === "create") {
      const res = await fetch(`${API_BASE_URL_V1}/tasks/create`, {
        method: "POST",
        credentials: "include",
        headers: {
      ...
      if (res.status === 200) {
        ...
        // use response.insertedId is not return task on single response
        navigate(`/tasks/${response._id}`);
      } else {
    ...
    if (type === "update") {
      delete values._id;
      const res = await fetch(`${API_BASE_URL_V1}/tasks/${task._id}`, {
        method: "PATCH",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
