...
func (s *Store) CreateBooking(ctx context.Context, booking *Booking, opts ...CreateOption) error {
	...
	insertBooking := sb.Insert("bookings").
		Columns("id", "concert_id", "batch_id", "price", "currency", "status", "created_at", "updated_at", "cust_name", "cust_email", "cust_phone").
		Values(booking.ID, booking.Concert.ID, booking.Batch.ID,
...
func (s *Store) FindBookingByID(ctx context.Context, ID string, opts ...FindOption) (*Booking, error) {
	...
	query := sb.Select("b.id", "c.id", "cb.id", "b.price", "b.currency", "b.status",
		...
		"c.name", "c.slug", "cb.name", "cb.start_time", "cb.end_time").
		From("bookings b").
		LeftJoin("concerts c ON b.concert_id = c.id").
		LeftJoin("concert_batches cb ON b.batch_id = cb.id").
		Where(sq.Eq{"b.id": ID, "b.deleted_at": nil}).
		PlaceholderFormat(sq.Dollar)

	err := query.QueryRowContext(ctx).
		Scan(&b.ID, &b.Concert.ID, &b.Batch.ID, &b.Price, &b.Currency, &b.Status,
			...
			&b.Concert.Name, &b.Concert.Slug, &b.Batch.Name, &b.Batch.StartTime, &b.Batch.EndTime)
	if err != nil {
		return nil, err
	}
...
func (s *Store) FindAllBookings(ctx context.Context, opts ...ListOption) ([]Booking, string, error) {
	...
	query := sb.Select("b.id", "c.id", "cb.id", "b.price", "b.currency", "b.status",
		...
		From("bookings b").
		LeftJoin("concerts c ON b.concert_id = c.id").
		LeftJoin("concert_batches cb ON b.concert_batch_id = cb.id").
		Where(filter).
	...
	for rows.Next() {
		...
		if err := rows.
			Scan(&b.ID, &b.Concert.ID, &b.Batch.ID, &b.Price, &b.Currency, &b.Status,
				&b.ReservedAt, &b.ExpiredAt, &b.PaidAt, &b.CreatedAt, &b.UpdatedAt, &b.Version,
				&b.Customer.Name, &b.Customer.Email, &b.Customer.Phone, &b.InvoiceNumber, &b.PaymentType,
				&b.Concert.Name, &b.Concert.Slug, &b.Batch.Name, &b.Batch.StartTime, &b.Batch.EndTime); err != nil {
			return nil, "", err
		}
