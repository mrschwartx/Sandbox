...
func (s Service) CreateBooking(ctx context.Context, req *v1.CreateBookingParam) (*Booking, error) {
	course, err := s.catalogStore.FindConcertByID(ctx, req.Booking.GetConcert())
	if err != nil {
		return nil, err
	}

	batch, err := s.catalogStore.FindAllBatchesByConcertID(ctx, req.Booking.GetBatch())
	if err != nil {
		return nil, err
	}
