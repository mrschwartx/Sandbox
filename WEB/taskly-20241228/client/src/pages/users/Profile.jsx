...
export default function Profile() {
  ...
  const handleFileUpload = async (files) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL_V1}/users/upload/${user._id}`, {
        method: "POST",
        credentials: "include",
        body: formData,
      });
  ...
  const doSubmit = async (values) => {
    try {
      let newUrl = user.avatar;
      if (files.length > 0) {
        newUrl = await handleFileUpload(files);
        values.avatar = newUrl;
      } else {
        delete values.avatar;
      }

      const res = await fetch(`${API_BASE_URL_V1}/users/update/${user._id}`, {
        method: "PATCH",
        credentials: "include",
        headers: {
  ...
  const handleDeleteUser = async () => {
    try {
      const res = await fetch(`${API_BASE_URL_V1}/users/delete/${user._id}`, {
        method: "DELETE",
        credentials: "include",
      });
      const data = await res.json();
